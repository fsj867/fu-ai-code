...
public class RateLimitAspect {
    ...
    private void initRateLimiter(RRateLimiter rateLimiter, String key, RateLimit rateLimit) {
        ...
        try {
            boolean isLock = lock.tryLock(1, 2, TimeUnit.SECONDS); // 等待1秒，持有2秒
            if (!isLock) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR,"限流组件启动失败");
            }
            // 双重检查,防止并发问题
            if (!rateLimiter.isExists()) {
                boolean success = rateLimiter.trySetRate(RateType.OVERALL, rateLimit.rate(), rateLimit.rateInterval(), RateIntervalUnit.SECONDS);
                if (success) {
                    // 如果成功设置限流器，设置过期时间
                    rateLimiter.expire(Duration.ofSeconds(rateLimit.rateInterval() * 2L));
                }
